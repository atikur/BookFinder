...
class WishlistBook: NSManagedObject {

    convenience init(book: Book, context: NSManagedObjectContext) {
        if let entity = NSEntityDescription.entityForName("WishlistBook", inManagedObjectContext: context) {
            ...
            fatalError("Unable to find Entitiy 'WishlistBook'.")
        }
    }
}
